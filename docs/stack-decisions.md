...
- **OpenClaw Chat Completions HTTP API** — [ADR-XXXX](adr/XXXX-openclaw-chat-completions-http.md): Enable `chatCompletions` behind the existing nginx Route. OpenShell strips `Authorization` on sandbox services ([issue 1794](https://github.com/NVIDIA/OpenShell/issues/1794)); nginx copies the Bearer token to `X-OpenClaw-Authorization` and an in-sandbox proxy on `:18790` restores it. Number `XXXX` is assigned on merge to `main`.
