/**
 * v4 step 0 — full overall-demo-architecture FlowStory embed in-card.
 */
...
export async function mountOverallEmbed(container) {
  ...
  container.classList.add("nr-v4-overall-mounted");
  wireInDocOverlayClose();

  activeDiagram = withEmbedPaths(buildOverallDiagram());

  // Filter out steps referencing conditional nodes not present in v4
...
export function destroyOverallEmbed() {
  ...
  if (activeContainer) {
    activeContainer.classList.remove("nr-v4-overall-mounted");
